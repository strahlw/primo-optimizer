...
class WellDataColumnNames:  # pylint: disable=too-many-instance-attributes
    ...
    def return_column_names_to_summarize_count(self):
        """
        Returns the names of columns useful for a count in a project summary
        """
        # TODO: Support user specific columns
        return [
            col
            for col in [
                self.leak,
    ...
    def return_column_names_to_summarize_stats(self):
        """
        Returns the names of the columns for a simple statistical view in a project summary
        """
        # TODO: Support user specific columns
        return [
            col
            for col in [
                self.age,
                self.depth,
                self.ann_gas_production,
                self.ann_oil_production,
                self.five_year_gas_production,
                self.five_year_oil_production,
                self.life_gas_production,
                self.life_oil_production,
                self.state_dac,
                self.hospitals,
                self.schools,
                self.buildings_far,
                self.buildings_near,
                self.fed_wetlands_far,
                self.fed_wetlands_near,
                self.h2s_leak,
            ]
            if col is not None
        ]
